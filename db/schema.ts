...
export const rooms=sqliteTable("rooms",{code:text("code").primaryKey(),host:text("host").notNull(),created:integer("created").notNull(),expires:integer("expires").notNull(),state:text("state"),updated:integer("updated").notNull(),public:integer("public").notNull().default(1)});
