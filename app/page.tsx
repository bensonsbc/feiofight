...
const specials={hiro:CHARACTERS.hiro.special,marica:CHARACTERS.marica.special,lobao:CHARACTERS.lobao.special,ratao:CHARACTERS.ratao.special,bale:CHARACTERS.bale.special,veio:CHARACTERS.veio.special};
...
export default function Home(){
 ...
 useEffect(()=>{
  ...
  function tick(now:number){for(const k of Object.keys(pulse.current) as (keyof Input)[]){if(!held.current[k] && now >= (pulse.current[k]||0)){input.current[k]=false;delete pulse.current[k]}} const dt=Math.min((now-last)/1000,.1);last=now;const n=net.current,s=state.current,isHost=sessionRef.current?.slot===0;
   if(isHost){const rival=n?.members.find(m=>m.slot===1);if(isHero(rival?.hero)&&s.fighters[1].hero!==rival.hero){const fresh=createState(s.fighters[0].hero,rival.hero);fresh.tick=s.tick+1;Object.assign(s,fresh)}s.connected=!!n?.opponentPresent;s.paused=!visible.current||!s.connected;if(s.connected&&s.phase==="waiting")begin(s)}
 ...
 async function connect(op:"create"|"join",watch=false){
  ...
  try{const s:Session=await roomRequest({op,name:name.trim()||(watch?"Visitante":"Jogador"),hero,code:code.trim().toUpperCase(),watch});
   state.current=createState(s.hostHero,s.slot===1?s.hero!:undefined);sessionRef.current=s;setSession(s);setTraining(false);trainingRef.current=false;input.current=emptyInput();
  ...
  <section className="game-layout">
   <div className="arena-column">
    <div className="stage-shell" ref={stage}>
     ...
     {loaded&&!active&&<div className="attract"><span>{title(view.fighters[0].hero)} <em>vs</em> {title(view.fighters[1].hero)}</span><p>Escolha seu lado. Convide seu rival.</p></div>}
