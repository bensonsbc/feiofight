...
const title=(h:string)=>h==="hiro"?"HIRO":"MARICA";
const specials={hiro:"Galinha das Sombras",marica:"Brinde Sísmico"};
...
export default function Home(){
 ...
 const held=useRef<Input>(emptyInput()),pulse=useRef<Partial<Record<keyof Input,number>>>({}); const canvas=useRef<HTMLCanvasElement>(null),stage=useRef<HTMLDivElement>(null),state=useRef<State>(createState()),input=useRef<Input>(emptyInput()),net=useRef<RoomNetwork|null>(null),renderer=useRef<Renderer|null>(null),audio=useRef<AudioContext|null>(null),soundOn=useRef(false),sessionRef=useRef<Session|null>(null),trainingRef=useRef(false),visible=useRef(true);
 useEffect(()=>{soundOn.current=sound},[sound]);
 useEffect(()=>{const q=new URLSearchParams(location.search);if(q.get("sala"))setCode(q.get("sala")!.toUpperCase());return()=>net.current?.close()},[]);
 ...
 useEffect(()=>{
  ...
  function tick(now:number){for(const k of Object.keys(pulse.current) as (keyof Input)[]){if(!held.current[k] && now >= (pulse.current[k]||0)){input.current[k]=false;delete pulse.current[k]}} const dt=Math.min((now-last)/1000,.1);last=now;const n=net.current,s=state.current,isHost=sessionRef.current?.slot===0;
   if(isHost){s.connected=!!n?.opponentPresent;s.paused=!visible.current||!s.connected;if(s.connected&&s.phase==="waiting")begin(s)}
  ...
  frame=requestAnimationFrame(tick);
  const onKey=(e:KeyboardEvent)=>{if((e.target as HTMLElement)?.closest("input,textarea,select"))return;const key=keyMap[e.code];if(!key||(!sessionRef.current&& !trainingRef.current)||sessionRef.current?.slot===null)return;e.preventDefault();const down=e.type==="keydown";held.current[key]=down;if(down && ["jump","punch","kick","special"].includes(key))pulse.current[key]=performance.now()+120;input.current[key]=down||performance.now()<(pulse.current[key]||0);net.current?.sendInput(input.current)};
 ...
 useEffect(()=>{if(!session&&!training){state.current=createState(hero);setView({...state.current})}},[hero,session,training]);
 async function connect(op:"create"|"join",watch=false){
  if(!loaded)return;setBusy(true);setError("");
  try{const s:Session=await roomRequest({op,name:name.trim()||(watch?"Visitante":"Jogador"),hero,code:code.trim().toUpperCase(),watch});
   state.current=createState(s.slot===0?s.hero!:s.hero?(s.hero==="hiro"?"marica":"hiro"):"marica");sessionRef.current=s;setSession(s);setTraining(false);trainingRef.current=false;input.current=emptyInput();
 ...
 function leave(){net.current?.close();net.current=null;sessionRef.current=null;setSession(null);trainingRef.current=false;setTraining(false);input.current=emptyInput();state.current=createState(hero);setMembers([]);setOnlineError("");setFallback(false);history.replaceState(null,"",location.pathname)}
 ...
 function action(key:keyof Input,down:boolean){input.current[key]=down}
  ...
  <section className="game-layout">
   <div className="arena-column">
    <div className="stage-shell" ref={stage}>
     ...
     {loaded&&!active&&<div className="attract"><span>MARICA <em>vs</em> HIRO</span><p>Escolha seu lado. Convide seu rival.</p></div>}
     ...
     <div className="energy-hud">{view.fighters.map((f,i)=><div key={i} className={"energy-side energy-"+i}><div className={"energy "+(f.energy>=60?"charged":"")}><div style={{width:f.energy+"%"}}/></div><span>{f.energy>=60?"ESPECIAL PRONTO":specials[f.hero].toUpperCase()}</span></div>)}</div>
    </div>
    <div className="stage-caption"><span>ESQUINA DA TURMA <span className="caption-divider">/</span> MELHOR DE 3</span><button onClick={()=>stage.current?.requestFullscreen?.()} aria-label="Abrir arena em tela cheia">TELA CHEIA ⛶</button></div>
   ...
   <aside className="lobby">
    {!active?<><p className="eyebrow">ENTRE NA LUTA</p><h1>Escolha seu<br/>personagem.</h1><div className="hero-options">{(["marica","hiro"] as Hero[]).map(h=><button key={h} aria-pressed={hero===h} className={hero===h?"selected":""} onClick={()=>setHero(h)}><span className="hero-number">{h==="marica"?"01":"02"}</span>{title(h)}</button>)}</div><div className={"special-info "+hero}><span>GOLPE ESPECIAL</span><strong>{specials[hero]}</strong><p>{hero==="marica"?"Uma onda dourada. Nenhuma gota derramada.":"Uma galinha preta invocada entre velas e energia violeta."}</p></div><label className="field-label" htmlFor="nickname">Seu apelido</label><input id="nickname" value={name} maxLength={20} onChange={e=>setName(e.target.value)} placeholder="Como a turma te chama?" autoComplete="nickname"/><button className="primary-button" disabled={busy||!loaded} onClick={()=>connect("create")}>{busy?"CONECTANDO…":"CRIAR SALA"}<span>＋</span></button><div className="or-divider"><span>JÁ TEM UM CÓDIGO?</span></div><label className="sr-only" htmlFor="room-code">Código da sala</label><input id="room-code" className="code-input" value={code} maxLength={8} onChange={e=>setCode(e.target.value.toUpperCase().replace(/[^A-Z2-9]/g,""))} placeholder="CÓDIGO DA SALA" autoComplete="off"/><div className="join-actions"><button disabled={busy||code.length!==8||!loaded} onClick={()=>connect("join")}>ENTRAR NA LUTA</button><button disabled={busy||code.length!==8||!loaded} onClick={()=>connect("join",true)}>ASSISTIR</button></div><p className="join-hint">Ao entrar, você assume o personagem disponível.</p><button className="training-link" disabled={!loaded} onClick={train}>Testar os controles sozinho</button></>:<>
