...
import {createBrain,ladder,think,type Brain} from "../lib/ai";
...
// Solo ladder: beat every other fighter in order. `result` is what the overlay is showing between fights.
type Arcade={ladder:Hero[];stage:number;result:"fight"|"won"|"lost"|"champion"};
...
export default function Home(){
 ...
 const held=useRef<Input>(emptyInput()),pulse=useRef<Partial<Record<keyof Input,number>>>({}); const canvas=useRef<HTMLCanvasElement>(null),stage=useRef<HTMLDivElement>(null),state=useRef<State>(createState()),input=useRef<Input>(emptyInput()),net=useRef<RoomNetwork|null>(null),renderer=useRef<Renderer|null>(null),audio=useRef<AudioContext|null>(null),soundOn=useRef(false),sessionRef=useRef<Session|null>(null),trainingRef=useRef(false),visible=useRef(true),rivalId=useRef<string|null>(null),autoWatch=useRef(""),arcadeRef=useRef<Arcade|null>(null),brain=useRef<Brain>(createBrain());
 ...
 useEffect(()=>{
  ...
  function tick(now:number){for(const k of Object.keys(pulse.current) as (keyof Input)[]){if(!held.current[k] && now >= (pulse.current[k]||0)){input.current[k]=false;delete pulse.current[k]}} const dt=Math.min((now-last)/1000,.1);last=now;const n=net.current,s=state.current,isHost=sessionRef.current?.slot===0;
   ...
   if(trainingRef.current||arc){s.connected=true;s.paused=!visible.current;if(s.phase==="waiting")begin(s)}
   if(isHost||trainingRef.current||arc){acc+=dt;const remote=n&&Date.now()-n.remoteAt<700?n.remoteInput:emptyInput();while(acc>=1/60){step(s,[input.current,arc?think(s,1,brain.current,1/60,arc.stage):remote]);acc-=1/60}if(n&&now-lastSend>33){n.broadcast(s);lastSend=now}}
   if(arc&&arc.result==="fight"&&s.phase==="over"){arc.result=s.winner===0?(arc.stage>=arc.ladder.length-1?"champion":"won"):"lost";setArcade({...arc})}
 ...
 function leave(){net.current?.close();net.current=null;sessionRef.current=null;rivalId.current=null;setSession(null);trainingRef.current=false;setTraining(false);arcadeRef.current=null;setArcade(null);input.current=emptyInput();state.current=createState(hero);setMembers([]);setOnlineError("");setFallback(false);history.replaceState(null,"",location.pathname)}
 ...
 // Arcade: fight the other six in catalog order. Each fight keeps `tick` growing like a rematch would.
 function startFight(arc:Arcade,fighter:Hero){const t=state.current.tick;state.current=createState(fighter,arc.ladder[arc.stage]);state.current.tick=t+1;input.current=emptyInput();begin(state.current);arcadeRef.current=arc;setArcade({...arc});canvas.current?.focus()}
 function startArcade(){setError("");trainingRef.current=false;setTraining(false);brain.current=createBrain(Date.now()>>>0);startFight({ladder:ladder(hero),stage:0,result:"fight"},hero)}
 function nextFight(){const arc=arcadeRef.current;if(!arc)return;startFight({...arc,stage:arc.stage+1,result:"fight"},state.current.fighters[0].hero)}
 function retryFight(){const arc=arcadeRef.current;if(!arc)return;startFight({...arc,result:"fight"},state.current.fighters[0].hero)}
 ...
 const isSpectator=session?.slot===null,active=!!session||training||!!arcade,host=session?.slot===0;
 const beaten=(i:number)=>!!arcade&&(i<arcade.stage||(i===arcade.stage&&arcade.result!=="fight"&&arcade.result!=="lost"));
 const overlay=active?(view.phase==="waiting"?"AGUARDANDO RIVAL":view.paused?"PARTIDA PAUSADA":view.phase==="countdown"?String(Math.max(1,Math.ceil(view.clock))):view.phase==="round"?(view.winner===-1?"EMPATE":"K.O."):view.phase==="over"?(arcade?(arcade.result==="lost"?"VOCÊ PERDEU":arcade.result==="champion"?"CAMPEÃO DA TURMA!":"VOCÊ VENCEU!"):"VITÓRIA DE "+title(view.fighters[view.winner]?.hero||"marica")):null):null;
  ...
  <section className="game-layout">
   ...
   <aside className="lobby">
    {!active?<><p className="eyebrow">ENTRE NA LUTA</p><h1>Escolha seu<br/>personagem.</h1><div className="hero-options">{HEROES.map(h=><button key={h} aria-pressed={hero===h} className={hero===h?"selected":""} disabled={room?.hostHero===h} title={room?.hostHero===h?"Já escolhido por "+room.hostName:undefined} onClick={()=>setHero(h)}><span className="hero-number">{CHARACTERS[h].number}</span>{title(h)}</button>)}</div><div className={"special-info "+hero}><span>GOLPE ESPECIAL</span><strong>{CHARACTERS[hero].special}</strong><p>{CHARACTERS[hero].description}</p></div><label className="field-label" htmlFor="nickname">Seu apelido</label><input id="nickname" value={name} maxLength={20} onChange={e=>setName(e.target.value)} placeholder="Como a turma te chama?" autoComplete="nickname"/><button className="primary-button" disabled={busy||!loaded} onClick={()=>connect("create")}>{busy?"CONECTANDO…":"CRIAR SALA"}<span>＋</span></button><div className="or-divider"><span>JÁ TEM UM CÓDIGO?</span></div><label className="sr-only" htmlFor="room-code">Código da sala</label><input id="room-code" className="code-input" value={code} maxLength={8} onChange={e=>setCode(e.target.value.toUpperCase().replace(/[^A-Z2-9]/g,""))} placeholder="CÓDIGO DA SALA" autoComplete="off"/><div className="join-actions"><button disabled={busy||code.length!==8||!loaded||!!room?.rivalName} onClick={()=>connect("join")}>ENTRAR NA LUTA</button><button disabled={busy||code.length!==8||!loaded} onClick={()=>connect("join",true)}>ASSISTIR</button></div><p className="join-hint">{room?(room.rivalName?"Os dois lugares já estão ocupados. Você ainda pode assistir.":room.hostName+" já escolheu "+title(room.hostHero)+". Escolha o seu lutador e entre."):"Escolha seu lutador antes de entrar. Cada personagem ocupa uma vaga."}</p><div className="or-divider"><span>SOZINHO</span></div><button className="secondary-button" disabled={!loaded} onClick={startArcade}>MODO ARCADE · VENÇA TODOS</button><button className="training-link" disabled={!loaded} onClick={train}>Testar os controles sozinho</button></>:<>
     ...
     {arcade&&<><p>Vença dois rounds para avançar. Cada adversário reage mais rápido e defende melhor que o anterior.</p><div className="room-members"><span className="field-label">ADVERSÁRIOS</span>{arcade.ladder.map((h,i)=><div className={"member "+(beaten(i)?"beaten":i===arcade.stage?"current":"")} key={h}><span className="player-tag">{beaten(i)?"✓":i+1}</span><div><strong>{title(h)}</strong><small>{beaten(i)?"vencido":i===arcade.stage?"agora":"em breve"}</small></div></div>)}</div></>}
     ...
     <button className="leave-button" onClick={leave}>{training?"VOLTAR":arcade?"DESISTIR":"SAIR DA SALA"}</button></>}
