...
export class Renderer{
 atlas!:Atlas;bg!:HTMLImageElement;chicken!:Sprite;ready=false;
 async load(){const [hiro,marica,defense,bg]=await Promise.all([image("/assets/hiro-sheet.png"),image("/assets/marica-sheet.png"),image("/assets/marica-defense.png"),image("/assets/arena.png")]);this.bg=bg;this.atlas={hiro:{},marica:{}};
 ...
 draw(ctx:CanvasRenderingContext2D,s:State,time:number){const W=960,H=540;ctx.imageSmoothingEnabled=false;ctx.clearRect(0,0,W,H);if(this.bg){ctx.drawImage(this.bg,0,0,W,H);ctx.fillStyle="rgba(3,7,19,.13)";ctx.fillRect(0,0,W,H)}if(!this.ready)return;
  ...
  for(const p of s.projectiles){ctx.save();ctx.translate(p.x,485-p.y);ctx.scale(p.dir,1);if(p.hero==="hiro"){const sp=this.chicken;ctx.shadowColor="#bb68ff";ctx.shadowBlur=18;ctx.drawImage(sp.canvas,-32,-45,76,98)}else{ctx.shadowColor="#ffcb53";ctx.shadowBlur=18;ctx.strokeStyle="#ffdc6f";ctx.lineWidth=9;ctx.beginPath();ctx.arc(-12,0,39,-1.25,1.25);ctx.stroke();ctx.strokeStyle="#fff3b2";ctx.lineWidth=3;ctx.beginPath();ctx.arc(-7,0,31,-1.15,1.15);ctx.stroke()}ctx.restore()}
