...
import {CHARACTERS,ROSTER_INFO,isHero,statsOf,type Roster} from "./characters";
...
export class Renderer{
 ...
 sprite(f:Fighter,s:State){const set=this.atlas[f.hero];if(!set)return null;let key:Action=f.action;let frame=0;if(key==="walk")frame=Math.floor(f.steps*9)%4;else if(key==="jump")frame=f.vy>100?1:2;else if(key==="crouch")frame=2;else if(key==="block")frame=f.flash>0?2:1;else if(["punch","kick","special"].includes(key)){const length=key==="punch"?.36:key==="kick"?.58:.95;frame=Math.min(3,Math.floor(f.time/length*4))}else if(key==="grab"){key="punch";frame=f.time<.14?1:2}else if(key==="thrown"){key="jump";frame=2}else if(key==="down"){key="jump";frame=2}else{key="crouch";frame=0}if(s.phase==="over"&&f.hp===0&&key!=="jump")frame=2;return set[key]?.[frame]??null}
 draw(ctx:CanvasRenderingContext2D,s:State,time:number){const W=960,H=540;ctx.imageSmoothingEnabled=false;ctx.clearRect(0,0,W,H);ctx.save();
  // Screen shake on heavy hits: the whole scene, background included, jolts by a few pixels.
  if(s.shake>0)ctx.translate(Math.round((Math.random()-.5)*s.shake*2),Math.round((Math.random()-.5)*s.shake*1.4));
  if(this.bg){ctx.drawImage(this.bg,-6,-6,W+12,H+12);ctx.fillStyle="rgba(3,7,19,.13)";ctx.fillRect(-6,-6,W+12,H+12)}if(!this.ready){ctx.restore();return}
  for(const f of s.fighters){const sp=this.sprite(f,s);if(!sp)continue;ctx.save();const size=statsOf(f.hero).height,lift=Math.min(1,f.y/260);ctx.fillStyle="rgba(0,0,0,"+(.45*(1-lift*.6)).toFixed(2)+")";ctx.beginPath();ctx.ellipse(f.x,485,49*size*(1-lift*.45),9*size*(1-lift*.45),0,0,Math.PI*2);ctx.fill();const breathe=f.action==="idle"?Math.sin(time*3)*1.3:0;ctx.translate(f.x,485-f.y+breathe);ctx.scale(f.face,1);
   // Knocked out: the stretched jump pose turned flat on the floor reads as lying on the back, head away from the winner.
   if(f.action==="down"||(f.action==="thrown"&&f.hp===0)){const flat=f.action==="down"?1:Math.min(1,f.time);ctx.translate(-sp.anchor*sp.scale*size*.2,0);ctx.rotate(flat*Math.PI/2*.92);ctx.translate(0,-8)}if(f.flash>0)ctx.filter=f.action==="block"?"brightness(1.6)":"brightness(2) sepia(.7)";ctx.drawImage(sp.canvas,-sp.anchor*sp.scale*size,-sp.base*sp.scale*size,sp.canvas.width*sp.scale*size,sp.canvas.height*sp.scale*size);ctx.restore()}
  ...
  // Combo counter over the attacker while the chain is alive.
  for(const f of s.fighters)if(f.combo>=2&&f.comboTime>0){ctx.save();ctx.font="900 26px Consolas,monospace";ctx.textAlign="center";ctx.fillStyle="#120e21";ctx.fillText(f.combo+" HITS",f.x+3,485-f.y-205+3);ctx.fillStyle=isHero(f.hero)?CHARACTERS[f.hero].color:"#fff";ctx.fillText(f.combo+" HITS",f.x,485-f.y-205);ctx.restore()}
  ctx.restore();
  // White flash when a special connects.
  if(s.flashFx>0){ctx.fillStyle="rgba(255,255,255,"+Math.min(.4,s.flashFx*3.2).toFixed(2)+")";ctx.fillRect(0,0,W,H)}
 }
}
